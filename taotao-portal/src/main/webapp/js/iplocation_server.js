...
//    var loguri = "http://csc.tt.com/log.ashx?type1=$type1$&type2=$type2$&data=$data$&pin=$pin$&referrer=$referrer$&jinfo=$jinfo$&callback=?";
...
//    var traceurl = ("https:" == document.location.protocol ? "https://mercuryssl" : "http://mercury") + ".tt.com/log.gif"
//                    + "?t=other.000000"
//                    + "&m=UA-J2011-1"
...
//            +'</li>'
//            +'{/for}';
//        var url = 'http://d.tt.com/nostockrecomm/get?productId=' + pageConfig.product.skuid,
//            _this = this,
...
//        });
    ...
    itemOver: function() {
        ...
        var urlRelated = 'http://simigoods.tt.com/SoldOutRecJsonData.aspx?ip='+ getCookie("ipLocation") +'&wids='+ pageConfig.product.skuid;
        var urlBroswerBuy = 'http://simigoods.tt.com/ThreeCCombineBuying/ThreeCBroswerBroswerJsonData.aspx?ip=' + getCookie('ipLocation') + '&wids=' + pageConfig.product.skuid;
        var _this = this;
    ...
    noItemNoData: function( isItemOver ) {
        ...
        // 没有推荐数据时调用新数据接口
        var url = 'http://simigoods.tt.com/SoldOutRecJsonData.aspx?ip='+ getCookie("ipLocation") +'&wids='+ pageConfig.product.skuid,
            _this = this;
...
var stockServiceDomain = "http://st.3.cn",rmsurl="http://rms.shop.tt.com",dcashurl="http://yfei.shop.tt.com/json/pop/fare.action";
//try{if(location.href.indexOf("localtest=true")>0){stockServiceDomain = "http://webstock.tt.com";rmsurl="http://rms.shop.360buy.net";dcashurl="http://yfei.shop.jd.net/json/pop/fare.action";}}catch(e){}
//cookie operate
function getCookie(name) {var arr = document.cookie.match(new RegExp("(^| )" + name + "=([^;]*)(;|$)"));if (arr != null) return unescape(arr[2]);return null;}
function setNewCookie(name,value,expires,path,domain,secure){if(!path){path="/"}if(!domain){domain="tt.com"}if(!secure){secure=false}var today=new Date();today.setTime(today.getTime());if(expires){expires=expires*1000*60*60*24}var expires_date=new Date(today.getTime()+(expires));document.cookie=name+'='+escape(value)+((expires)?';expires='+expires_date.toGMTString():'')+((path)?';path='+path:'')+((domain)?';domain='+domain:'')+((secure)?';secure':'')};function deleteCookie(name,path,domain){if(getCookie(name))document.cookie=name+'='+((path)?';path='+path:'')+((domain)?';domain='+domain:'')+';expires=Thu, 01-Jan-1970 00:00:01 GMT'};
...
$.getShopUrl=function(r){if(pageConfig.product.isFlashPurchase)return "#none";if(r.url)return r.url;return "http://mall.tt.com/index-"+r.id+".html";};
...
function flashPurchaseChange(){
	...
	if($('#summary-countdown').length==0){
		$.ajax({
			url: 'http://rms.shop.tt.com/json/upDown/queryWareTaskUpDownTimeBySkuId.action?skuId=' + pageConfig.product.skuid,
			dataType: 'jsonp',
			success: function(r) {
...
function flashPurchaseChange1(){
	...
	if($('#summary-countdown').length==0){
		$.ajax({
			url: 'http://red.tt.com/item/remainTime.html?skuid=' + pageConfig.product.skuid,
			dataType: 'jsonp',
			success: function(r) {
				if(r){
...
function getDeliveCash(r){
    ...
    if(r){
        if(r.dtype == 0&&new Number(r.dcash)>0){
            $("#store-prompt span").html("，运费："+r.dcash+"元 <a href='http://help.tt.com/help/question-892.html#help2215' class='free_delivery_policy' title='了解配送费收取标准'></a>");
        }
        else if(r.dtype == 1&&new Number(r.dcash)>0&&new Number(r.ordermin)>0){
            $("#store-prompt span").html("，店铺单笔订单不满"+r.ordermin+"元，收运费："+r.dcash+"元 <a href='http://help.tt.com/help/question-892.html#help2215' class='free_delivery_policy'></a>");
        }
        if(typeof $.fn.Jtips !== 'undefined'){
             $('.free_delivery_policy').Jtips({
                ...
                oLeft: -8,
                position:'bottom',
                content:'<a href="http://help.tt.com/help/question-892.html#help2215" target="_blank">了解配送费收取标准</a>',
                event: 'mouseover',
                close: true
            });
        }
    ...
    if (requestDeliveCash){
        try{
			if(dcashurl){
				...
				$.getJSONP("http://fare.shop.tt.com/json/pop/fare.action?venderId="+pageConfig.product.venderId+"&skuId="+pageConfig.product.skuid+"&provinceId="+currentAreaInfo.currentProvinceId+"&cityId="+currentAreaInfo.currentCityId+"&buyNum="+$("#buy-num").val()+"&callback=getDeliveCash");
			}
        }catch(e){}
    }
}
...
function getBuyUrl(skuId){
    ...
    if(!count)count=1;
	if(pageConfig.product.isHYKHSP)return "http://eve.tt.com/redirect.action?wid="+skuId+"&btype=30";
	if(pageConfig.product.isLOC){
		return "http://cart.tt.com/cart/dynamic/gateForSubFlow.action?wids="+skuId+"&nums="+count+"&subType=22";
	}
    if(eleSkuIdKey) return "http://gate.tt.com/InitCart.aspx?pid="+skuId+"&pcount="+count+"&ptype=1";
    if($.pbuyurl)return $.pbuyurl;
    if($.append_button.attr("href")!="#none")return $.append_button.attr("href");
    if (pageConfig.product.cat[2] == 4833) return "http://chongzhi.tt.com/order/order_place.action?skuId=" + skuId + "";
    if (pageConfig.product.cat[2] == 4835 || pageConfig.product.cat[2] == 4836) return "http://card.tt.com/order/order_place.action?skuId=" + skuId + "";
    return "http://gate.tt.com/InitCart.aspx?pid="+skuId+"&pcount="+count+"&ptype=1";
}
function chooseType() {
...
function setGiftTips (isGift, areaText) {
	if(checkApecialAttr("isSupportCard")){
		...
		var giftTipsHtml = (areaText || '北京地区') + '支持礼品包装' + '<a href="http://cart.gift.tt.com/cart/addGiftToCart.action?pid='+pageConfig.product.skuid+'&pcount=1&ptype=1" target="_blank">&nbsp;&nbsp;详情 <s class="s-arrow">&gt;&gt;</s></a>';
		if (isGift) {
...
//			var newUrl = "http://eve.tt.com/redirect.action?wid="+pageConfig.product.skuid+"&btype=16&pid="+currentAreaInfo.currentProvinceId+"&cid="+currentAreaInfo.currentCityId;
...
//        $("<div id='choose-btn-notice' class='btn'><a id='notify-btn' class='btn-notice' href='http://notify.home.tt.com/email.action?type=2&id=" + pageConfig.product.skuid + "&key=" + pageConfig.product.skuidkey + "' target='_blank'>到货通知<b></b></a></div>").insertAfter("#choose-btn-append");
...
function setOldForNew(proId,cityId){
	...
	if (pageConfig.product.cat[2]==655){
		if ($("#oldfornew-tips").length==0){
			summary_tips.find(".dd").append("<div id=\"oldfornew-tips\" class=\"hl_red\">以旧换新，最高抵扣3000元<a target=\"_blank\" href=\"http://huishou.tt.com\" clstag=\"shangpin|keycount|product|huishou\">&nbsp;&nbsp;详情<s class=\"s-arrow\">>></s></a></div>");
		}else{
			$("#oldfornew-tips").show();
		}
		summary_tips.show();
...
function getChildAreaHtml(fid,level){
    ...
    if (idName){
        ...
        if(level == 2 && provinceCityJson[fid+""]){
            ...
            $.getJSONP("http://d.tt.com/area/get?fid="+fid+"&callback=getAreaListcallback");
        }
    }
}
...
function aboutSelfDeliveCash(type,flag){
    if((pageConfig.product.skuid+"").length<10||flag){
        if(dCashDescInfo.loadPriceCnt==dCashDescInfo.loadStockCnt&&dCashDescInfo.bigger39&&!dCashDescInfo.dCash){
            if(type == 1){ //stock
                return '<a href="http://help.tt.com/help/question-892.html#help2215" target="_blank" class="free_delivery" title="了解配送费收取标准"></a>';
            }
            else if (type == 2){ //price
                if($("#promise-ico").length>0){
                    if($("#promise-ico .free_delivery").length==0){
                        $('<a href="http://help.tt.com/help/question-892.html#help2215" target="_blank" class="free_delivery" title="了解配送费收取标准"></a>').appendTo("#promise-ico");
                    }
                }
                else{
                    $('<span id="promise-ico">支持：<a href="http://help.tt.com/help/question-892.html#help2215" target="_blank" class="free_delivery" title="了解配送费收取标准"></a></span>').appendTo("#summary-service .dd");
                }
            }
        }
    }
...
/*var checkLogin = function(cb) {
	...
	$.getJSON('http://passport.tt.com/loginservice.aspx?method=Login&callback=?', function(r) {
		if ( r.Identity ) {
			cb(r.Identity);
		}
	});
						...
						$.extend(jdModelCallCenter.settings, {
							...
							fn: function () {
								$.login({
									...
									complete: function (result) {
										if (result != null && result.IsAuthenticated != null && result.IsAuthenticated) {								
											$.getJSONP("http://easybuy.tt.com/skuDetail/submitEasybuyOrder.action?callback=easybuysubmit&skuId="+pageConfig.product.skuid+"&num="+$("#buy-num").val());
										}
										else{
											$("#btn-easybuy-submit").show();
											$("#btn-easybuy-submit-ing").hide();
    ...
    if(r&&r.resultCode==1&&r.skuId&&r.skuId!=pageConfig.product.skuid+''){
		...
		if((r.skuId+"").length==10){
			if($("#local-seller").length==0){
				$('<div class="mt" id="local-seller"><span class="fl"><b>本地在售卖家</b></span></div><div class="mc" id="local-seller1"><ul><li id="J_'+r.skuId+'"><div class="fl"><a target="_blank" href="http://item.tt.com/'
			...
			$('<div class="mt" id="jd-seller"><span class="fl"><b>其它在售卖家</b></span></div><div class="mc" id="jd-seller1"><ul><li id="J_'+r.skuId+'"><div class="fl"><a target="_blank" href="http://item.tt.com/'
        ...
        try{
            ...
            if (curLevel == requestLevel){
                ...
                if(pageConfig.product.TJ=='1'&&provinceId>0&&cityId>0&&areaId>0){
                    $.getJSONP("http://d.tt.com/goodsrelation/get?callback=getTJAreaSku&skuId="+pageConfig.product.skuid+"&provinceId="+provinceId+"&cityId="+cityId+"&countryId="+areaId);
                }
...
//    }
//    else{
//        $.ajax({url:"http://uprofile.tt.com/u/setadds",type:"get",dataType:"jsonp",data:{adds:adds}});
//    }
}
//根据省份ID获取名称
...
//			try{
//				$.ajax({
//					url:"http://uprofile.tt.com/u/getadds",
//					type:"get",
//					dataType:"jsonp",
//					timeout:500,
//					success:function(r){
...
var spuServiceUrl = "http://spu.tt.com/json.html?cond=";
var spuPageUrl = "http://spu.tt.com/"+pageConfig.product.skuid+".html";
    ...
    if(window.itemShopJsonb&&itemShopJsonb.hotSale&&itemShopJsonb.hotSale.length==5){
        ...
        for (var i=0,j=itemShopJsonb.hotSale.length;i<j;i++){
            hotSaleContent.push('<li class="fore'+(i+1)+'">'
                            +'<div class="p-img"><a href="http://item.tt.com/'+itemShopJsonb.hotSale[i].skuId+'.html" title="'+itemShopJsonb.hotSale[i].goodsName+'" target="_blank"><img alt="'+itemShopJsonb.hotSale[i].goodsName+'" data-lazyload="'+getImgFullPath(itemShopJsonb.hotSale[i].skuId,itemShopJsonb.hotSale[i].goodsImg,2)+'"></a></div>'
                            +'<div class="p-name"><a href="http://item.tt.com/'+itemShopJsonb.hotSale[i].skuId+'.html" target="_blank" title="'+itemShopJsonb.hotSale[i].goodsName+'">'+itemShopJsonb.hotSale[i].goodsName+'</a></div>'
    ...
    if(window.itemShopJsonb&&itemShopJsonb.hotFocus&&itemShopJsonb.hotFocus.length==5){
        ...
        for (var i=0,j=itemShopJsonb.hotFocus.length;i<j;i++){
            hotFocusContent.push('<li class="fore'+(i+1)+'">'
                            +'<div class="p-img"><a href="http://item.tt.com/'+itemShopJsonb.hotFocus[i].skuId+'.html" title="'+itemShopJsonb.hotFocus[i].goodsName+'" target="_blank"><img alt="'+itemShopJsonb.hotFocus[i].goodsName+'" data-lazyload="'+getImgFullPath(itemShopJsonb.hotFocus[i].skuId,itemShopJsonb.hotFocus[i].goodsImg,2)+'"></a></div>'
                            +'<div class="p-name"><a href="http://item.tt.com/'+itemShopJsonb.hotFocus[i].skuId+'.html" target="_blank" title="'+itemShopJsonb.hotFocus[i].goodsName+'">'+itemShopJsonb.hotFocus[i].goodsName+'</a></div>'
                            +'<div class="p-info p-bfc">'
    ...
    if(window.itemShopJsona&&itemShopJsona.shopRec&&itemShopJsona.shopRec.length>0){
        ...
        for (var i=0,j=itemShopJsona.shopRec.length;i<j;i++){
            shopRecContent.push('<li class="fore'+(i+1)+'">'
                            +'<div class="p-img"><a href="http://item.tt.com/'+itemShopJsona.shopRec[i].skuId+'.html" title="'+itemShopJsona.shopRec[i].goodsName+'" target="_blank"><img alt="'+itemShopJsona.shopRec[i].goodsName+'" data-lazyload="'+getImgFullPath(itemShopJsona.shopRec[i].skuId,itemShopJsona.shopRec[i].goodsImg,2)+'"></a></div>'
                            +'<div class="p-name"><a href="http://item.tt.com/'+itemShopJsona.shopRec[i].skuId+'.html" target="_blank" title="'+itemShopJsona.shopRec[i].goodsName+'">'+itemShopJsona.shopRec[i].goodsName+'</a></div>'
                            +'<div class="p-info p-bfc">'
...
//	$.ajax({type:"get",
//			dataType:"jsonp",
//			url:"http://x.tt.com/ShowInterface?ad_ids=57:1&urlcid3="+pageConfig.product.cat[2]+"&ad_type=8&spread_type=1&debug=0&location_info=0",
//			success:function(data){
//				 if (data) {
//					if (data.errcode == 0) {
//						try{
